...
void wifi_sniffer_packet_handler(void* buff, wifi_promiscuous_pkt_type_t type){
	...
	if ((pacchetto.getTypeMessage() == 0) && (pacchetto.getSubTypeMessage() == 4)) {
		...
		ESP_LOGD(tag, "Forza segnale: %d", pacchetto.getSignalStrength());
		ESP_LOGD(tag, "Source address: %s", pacchetto.getSourceMacAddress().c_str());
		if (pacchetto.getSSID() != "") {
			ESP_LOGD(tag, "SSID: %s", pacchetto.getSSID().c_str());
		}
		printf("\n");
	}
